...
def build_styles() -> dict:
    ...
    s["bullet"] = ParagraphStyle("bullet", parent=s["p"], alignment=0, spaceAfter=2.5, leading=13.4)
    ...
    return [
        ...
        table_block(
            [
                ...
                [
                    "Stack",
                    "Python 3.12 · FastAPI · Docker · Kubernetes · Prometheus · Grafana · GitHub Actions",
                ],
                [
                    "Estate monitored",
                    "19 resources — 16 simulated Azure/AWS, 3 live Docker containers",
                ],
                ...
                [
                    "Headline result",
                    "$7,532/month estate · 55% waste identified · ~$55,000/year in findings",
                ],
            ],
            [0.24, 0.76],
        ),
...
def document() -> list[tuple]:
            ...
            (
                [
                    ...
                    [
                        ...
                        "cgroup CPU/memory, scraped over HTTP",
                    ],
                    [
                        "Chaos",
                        "Metric modifiers",
                        "Genuinely burns CPU, leaks memory, returns 500s, exits and is restarted",
                    ],
                    [
                        "Provides",
                        "Breadth: RDS, S3, Lambda, load balancers, orphaned disks, two clouds, real SKUs, regions, tags",
    ...
    add(
        (
            "p",
            "<font color='#1a7f37'><b>All end-to-end checks pass</b></font> (53/53 on the run below) against the running "
            ...
            (
                [
                    ...
                    [
                        "Pull (scrape)",
                        "Push",
                        "A target that dies is detected by its <i>absence</i>. With push, a silent target and a healthy target look identical.",
                    ],
                    [
                        "SQLite (WAL)",
                        "Postgres/Timescale",
                        "Must run on a laptop. Few hundred rows per tick. Append-only series + small mutable tables ports upward unchanged. Cost: single writer.",
                    ],
                    [
                        "Robust z-score (median + MAD)",
                        "Mean + stdev, or ML",
                        "Explainable at 3am, no training job, cold-start safe. One outlier inflates a stdev enough to hide the next real anomaly.",
                    ],
                    [
                        "Deterministic simulator",
                        "random per sample",
                        "value(resource, metric, t) is a pure function, so backfill is continuous with live data and the demo reproduces exactly. White noise would make any detector look good.",
                    ],
                    [
                        "Rule-based recommendations",
                        "ML sizing model",
                        'Every finding shows its evidence. "The model said so" does not survive review by the team that owns the resource.',
                    ],
                    [
                        "Vanilla-JS dashboard",
                        "React + chart library",
                        "No build step, no CDN, no npm dependency tree bolted onto a security tool. Works air-gapped.",
                    ],
                    [
                        "No Docker socket",
                        "Mount /var/run/docker.sock",
                        "That socket is root on the host. A restart counter is not worth it — start-time inference is used instead.",
                    ],
                ],
                [0.24, 0.2, 0.56],
            ),
            ...
            (
                [
                    ...
                    [
                        ...
                        "Subscription",
                        "Account",
                        "<font face='Courier'>subscriptions:</font> in inventory.yaml",
                    ],
                    [
                        "Grouping",
                        "Resource group",
                        "Tags / resource groups",
                        "<font face='Courier'>resource_group</font>",
                    ],
                    ...
                    [
                        ...
                        "Lambda",
                        "serverless_function, free tier modelled",
                    ],
                    [
                        "Managed SQL",
                        "Azure SQL (GP_Gen5_8)",
                        "RDS (db.r6g.4xlarge)",
                        "managed_database, 1.15x premium",
                    ],
                    ...
                    [
                        ...
                        "ALB / NLB",
                        "load_balancer, fixed + per-GB",
                    ],
                    [
                        "Workload identity",
                        "Managed Identity",
                        "IAM Role / IRSA",
                        "<font face='Courier'>managed_identity</font> posture flag",
                    ],
                    [
                        "Secret store",
                        "Key Vault",
                        "Secrets Manager",
                        "Recommended over K8s Secrets",
                    ],
            ...
            (
                [
                    ...
                    [
                        ...
                        "Keep waiting (30 x 5s here)",
                        "No",
                    ],
                    [
                        "livenessProbe",
                        "Is the process wedged?",
                    ...
                    [
                        ...
                        "Remove from the Service endpoints",
                        "Yes, legitimately",
                    ],
                ],
                [0.16, 0.26, 0.32, 0.26],
        ...
        (
            ...
            (
                [
                    ...
                    [
                        "lint",
                        "ruff lint + format; yamllint; and a config check that every resource type has a pricing entry (otherwise it silently costs $0 and vanishes from the FinOps view) and every alert rule has a runbook.",
                    ],
                    [
                        "test",
                        "69 unit tests with coverage; results uploaded even on failure — a red build with no visible reason is the worst CI experience.",
                    ],
                    [
                        "build",
                        "Both images build; <b>fails if either runs as root</b>; fails if either lacks a HEALTHCHECK; layer caching via type=gha.",
                    ],
                    [
                        "e2e",
                        "Boots the full stack, runs the same scripts/verify_local.py a developer runs — including injecting real chaos and asserting detection. Then checks the Prometheus exposition and that every log line is valid JSON with the required fields.",
                    ],
                    [
                        "k8s-validate",
                    ...
                    [
                        "security",
                        "Trivy on images and filesystem, SARIF to the security tab. <b>Secret scanning is a hard failure; base-image CVEs are not.</b>",
                    ],
                    [
                        "publish",
                        "GHCR via job-scoped GITHUB_TOKEN, with SBOM and provenance. main branch pushes only — a fork PR can never push an image.",
                    ],
                ],
                [0.16, 0.84],
            ),
            ...
            (
                [
                    ...
                    [
                        ...
                        "gauge %",
                        "Saturation",
                        "Drives rightsizing and the HPA; % of the container limit, not the host",
                    ],
                    [
                        "memory_utilization",
                        "gauge %",
                        "Saturation",
                        "Predicts OOM kills — the failure that looks like a crash, not a slowdown",
                    ],
                    [
                        "disk_utilization",
                        "gauge %",
                        "Saturation",
                        "A full volume fails writes with errors that name nothing useful",
                    ],
                    [
                        "latency_p95_ms",
                        "gauge ms",
                        "Latency",
                        "p95, not mean — an average hides the tail users actually experience",
                    ],
                    [
                        "error_rate",
                        "ratio",
                        "Errors",
                        "Computed from counter deltas, so it is a rate not a lifetime average",
                    ],
                    [
                        "requests_per_second",
                        "gauge",
                        "Traffic",
                        'Distinguishes "load" from "fault" when CPU rises',
                    ],
                    [
                        "restart_count",
                        "counter",
                        "Reliability",
                        "Crash loops; must be alerted with increase(), never absolutely",
                    ],
                    [
                        "availability",
                        "gauge 0/1",
                        "Reliability",
                        "A failed scrape records 0 — down is a measurement",
                    ],
                    [
                        "estimated_cost / waste",
                        "gauge USD",
                        "Cost",
                        "Makes spend a first-class monitored signal, not a monthly surprise",
                    ],
                ],
                [0.2, 0.11, 0.13, 0.56],
            ),
...
z       = (current - median) / scale""",
    ...
    add(("p", "Two refinements, both of which came out of test failures rather than theory:"))
    add(
        (
            "bullets",
            [
        ...
        (
            ...
            (
                [
                    ...
                    [
                        "over_provisioned",
                        "p95 CPU 10-40%, 8+ samples",
                        "Resize down the SKU ladder",
                        "40-60%",
                    ],
                    [
                        "under_utilised",
                        "p95 CPU &lt;10% and memory &lt;20%",
                        "Decommission, or smallest SKU / spot",
                        "75%",
                    ],
                    [
                        "orphaned_resource",
                        "Disk or IP with attached: false",
                        "Snapshot, then delete",
                        "100%",
                    ],
                    [
                        "storage_tiering",
                        "&gt;500 GB hot, no lifecycle policy",
                        "Cool at 30d, archive at 180d",
                        "~45% of cold data",
                    ],
                    [
                        "scheduling",
                        "Non-prod compute &gt;$20/mo",
                        "Start/stop 08:00-20:00 Mon-Fri",
                        "64%",
                    ],
                    [
                        "commitment_discount",
                        "Prod, p95 CPU &gt;=40%, &gt;$50/mo",
                        "1-year RI / Savings Plan",
                        "28%",
                    ],
                    [
                        "unhealthy_container",
                        "3+ restarts or availability &lt;95%",
                        "Exit code, OOM status, probe thresholds",
                        "-",
                    ],
                    [
                        "high_error_rate",
                        "p95 error rate &gt;=3%",
                        "Correlate with deploys; roll back first",
                        "-",
                    ],
            ...
            (
                [
                    ...
                    [
                        ...
                        "rds-analytics — db.r6g.4xlarge at 9% CPU, $2,715/month",
                    ],
                ],
                [0.32, 0.68],
            ),
        ...
        (
            ...
            (
                [
                    ...
                    [
                        "make up / down / clean",
                        "Start · stop (history kept) · stop and wipe volumes",
                    ],
                    [
                        "make verify / verify-quick",
                        "Full end-to-end incl. live incident · without the 2-min incident test",
                    ],
                    ...
                    [
                        "make cost / recommendations / metrics",
                        "Cost summary · top findings · Prometheus exposition",
                    ],
                    ["make logs / ps", "Follow control plane logs · container status"],
                    [
                        "make k8s-validate / k8s-apply",
                        "Render + validate manifests · apply to current context",
                    ],
                ],
                [0.36, 0.64],
            ),
...
python scripts/verify_local.py --quick""",
    ...
    add(("p", '<i>For a recruiter, a manager, or the opening "tell me about your project".</i>'))
    add(
        (
            "note",
            ...
            "Grafana dashboard and a GitHub Actions pipeline. Sixty-nine unit tests, and a fifty-plus "
    ...
    add(
        (
            "p",
            '"Sixty-nine unit tests, a fifty-plus check end-to-end script that CI runs against real '
    ...
    add(
        (
            ...
            [
                ...
                "Achieved <b>69 unit tests and a 50+ check end-to-end verification suite</b> run identically by developers and CI; verification surfaced three latent defects including an alert on a monotonic counter that could never resolve.",
            ],
        )
    )
        ...
        (
            ...
            (
                [
                    ...
                    [
                        "Azure / AWS",
                        "Multi-cloud inventory, service equivalents, pricing models, managed identity/IRSA, Well-Architected pillars",
                    ],
                    [
                        "Docker",
                        "Multi-stage builds, non-root, capability dropping, health checks, cgroup measurement, Compose with anchors, secrets and profiles",
                    ],
                    [
                        "Kubernetes",
                        "StatefulSet vs Deployment, three probe types, requests/limits/QoS, HPA, PDB, quotas, NetworkPolicy, RBAC, PSA, kustomize",
                    ],
                    [
                        "CI/CD",
                        "7-stage pipeline, layer caching, artifact passing, SARIF, OIDC federation, gated environments, policy-as-CI-assertion",
                    ],
                    [
                        "Linux",
                        "cgroup v1/v2, /proc, signals and PID 1, process lifecycle, exit codes, file descriptors",
                    ],
                    [
                        "Monitoring",
                        "Golden signals, pull vs push, cardinality, recording rules, SLOs, multi-window burn rate, self-monitoring",
                    ],
                    [
                        "Logging",
                        "Structured JSON, correlation IDs, ContextVar, twelve-factor stdout, level discipline, bounded buffers",
                    ],
                    [
                        "REST APIs",
                        "Versioning, resource modelling, bounded pagination, correct status codes, OpenAPI, aggregate endpoints",
                    ],
                    [
                        "Python",
                        "asyncio, FastAPI, Pydantic, dataclasses, ContextVar, robust statistics, pytest with fixtures",
                    ],
                    [
                        "Security",
                        "Zero hardcoded credentials, constant-time comparison, least privilege, threat modelling, supply chain, XSS prevention",
                    ],
                    [
                        "Cost optimisation",
                        "Provisioned vs effective spend, waste quantification, safe rightsizing, commitments, tiering, tagging governance",
                    ],
                ],
                [0.19, 0.81],
            ),
        ...
        (
            ...
            (
                [
                    ...
                    [
                        "1",
                        "16 of 19 resources are simulated",
                        "Zero cloud spend was the hard constraint",
                        "Replace load_inventory() with Resource Graph / Resource Groups Tagging API — nothing downstream changes",
                    ],
                    [
                        "2",
                        "Cost model will not reconcile with an invoice",
                        "No EA, discounts, reservations, credits, egress, inter-AZ, NAT, snapshots, IOPS, support, tax",
                        "Retail Prices / Price List API for rates; Cost Management / CUR for actuals",
                    ],
                    [
                        "3",
                        "24-hour rightsizing window",
                        "Bounded by SQLite retention",
                        "14-30 days, after moving the store",
                    ],
                    [
                        "4",
                        "SQLite means one writer",
                        "Must run on a laptop",
                        "Postgres/Timescale — unblocks HA and horizontal scale together",
                    ],
                    [
                        "5",
                        "No downsampling; coarse pruning",
                        "Simplicity",
                        "Continuous aggregates or recording rules + a long-retention tier",
                    ],
                    [
                        "6",
                        "Restart detection is inferred",
                        "The authoritative source needs the Docker socket, which is root on the host",
                        "Read containerStatuses[].restartCount via the existing read-only Role",
                    ],
                    [
                        "7",
                        "No seasonality in detection",
                        "Explainability and cold-start safety chosen over sophistication",
                        "STL decomposition or Prophet",
                    ],
                    [
                        "8",
                        "Minimal alert routing; no inhibition",
                        "Scope",
                        "Alertmanager — the webhook payload is already Alertmanager-shaped",
                    ],
                    [
                        "9",
                        "Dashboard polls; no pagination",
                        "Simplicity at 19 resources",
                        "SSE, server-side pagination, virtualised rows",
                    ],
                    [
                        "10",
                        "K8s manifests validated, not deployed",
                        "A real cluster costs money; a kind cluster in CI tests kind",
                        "kind job in CI, plus a staging cluster",
                    ],
                    [
                        "11",
                        "Unauthenticated chaos endpoint",
                        "Real failure injection is the point of the demo",
                        "Compile out, or a separate authenticated admin port",
                    ],
                    [
                        "12",
                        "Single-tenant, no user model",
                        "Scope",
                        "OIDC, RBAC mapped to groups, per-user audit",
                    ],
                    [
                        "13",
                        "No load, soak or browser testing",
                        "Scope",
                        "Behaviour at 10,000 resources is genuinely unknown",
                    ],
                    [
                        "14",
                        "Only the cgroup v2 path is exercised",
                        "Developed on Docker Desktop",
                        "Test the v1 and /proc fallbacks on other hosts",
                    ],
                ],
                [0.04, 0.24, 0.32, 0.4],
            ),
